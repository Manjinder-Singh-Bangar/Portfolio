...
const ContactMe = () => {
  ...
  return (
    <section className='text-[#607B96] flex w-full h-[calc(100vh-104.2px)]'>
      <div className='min-w-[200px] flex-[0.6] '>
        <div className={` ${isContactOpened ? "pb-6 h-fit gap-3  border-b-[1px]" : "h-fit gap-0 border-b-0 p-0"} transition-all flex-col h-fit flex border-[#1E2D3D]`}>
          ...
          <div className={`flex transition-all gap-3 text-[16px] relative ${isContactOpened ? "top-[0]  opacity-1 h-fit" : "p-0 opacity-0 z-[-1] top-[-100px] h-0"} flex-col`}>
            <button className='flex gap-3 text-[12px] px-6'><img src={mailIconSvgSrc} alt="" /> manjinder@manjindersingh.ca</button>
            <button className='flex gap-3 text-[12px] px-6'><img src={telephoneIconSvgSrc} alt="" /> +1 437 661 5014</button>
          </div>
        </div>
      </div>

      <div className='h-full w-full flex flex-col flex-1 text-[#607B96] border-l-[1px] border-[#1E2D3D]'>
      ...
      <div className='h-full flex flex-col items-center border-l-[1px] border-[#1E2D3D]'>
